...
Finally, an expression for the photoelectron current $I_{ph}(\theta)$ can be expressed as 

\begin{equation}\label{eq:phCurrent}
    I_{ph}(\theta) = J_{ph}(0) A \cos \theta 
\end{equation}

Inserting equation \ref{eq:phCurrent} into equation \ref{eq:currentBalance}, the floating potential for a spacecraft subject to photoelectric and ambient plasma charging can be computed.
...
% \begin{figure}[h!]
%     \centering
%     \includegraphics[scale=0.70]{figures/blackbody.png}
%     \caption{Intensity of blackbody radiation for different temperatures}
%     \label{fig:blackbody}
% \end{figure}
