...
where $\rho$ is the electron density, $V_{cell}$ is the volume of the cell, and $N_{p,cell}$ is an input parameter specifying the number of particles per computational cell. Equation \ref{eq:NPHelectrons} can be rewritten to calculate the total number of photoelectrons ejected per timestep. Since several photoelectron injection methods have been analyzed for this thesis, the total photoelectron flux per timestep is the form that has been implemented into PINC
...
Listing \ref{lst:planck} shows a numeric implementation of equation \ref{eq:PlanckX} and equation \ref{eq:PlanckSum}. The original algorithm \parencite{Blackbody} has been converted from C++ code to C for compatibility with the PINC framework.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.45]{figures/DECA/NoPhotoelectrons/pNoPH.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.45]{figures/DECA/PhotoelectronsInjectionByCellFilling/pCellFill.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.45]{figures/DECA/PhotoelectronInjectionByRotationMatrix/pRotMat.png}
    \caption{Plot of the cubesat potential over time, in this case photoelectrons are injected into the domain using equation \ref{eq:injection} with velocity sampled from the univariate Maxwellian distribution and rotated using equation \ref{eq:rotMatFull}. The stippled horizontal lines show the floating potentials obtained by iPic3D and PTetra for the same simulation setup}
    \label{fig:pRotMat}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.45]{figures/DECA/PhotoelectronInjectionByDoubleGauss/pDoubleGauss.png}
    \caption{Plot of the cubesat potential over time, in this case photoelectrons are injected into the domain using equation \ref{eq:injection} and sampling the surface normal and tangential velocities separately. The stippled horizontal lines show the floating potentials obtained by iPic3D and PTetra for the same simulation setup}
